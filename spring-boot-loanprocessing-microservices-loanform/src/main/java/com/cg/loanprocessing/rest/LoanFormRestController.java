...
//	}// End of login
	...
	@PostMapping("/form/add")
	 public Response<LoanForm> addForms( @RequestBody LoanForm form) {
		// form.setLoanId(0);
		 form.setApplicationStatus("Requested");
		 form.setRole("ROLE_CUSTOMER");
		 LoanForm loanform =service.save(form);
		 if(loanform!=null) {
			 return new Response<LoanForm>(false," Successfull",loanform); 
		 }else {
			 return new Response<LoanForm>(true,"UnSuccessfull",null);
		 }
	 }
	@PutMapping("/changepassword/{email}/{password}")
	public Response<LoanForm>changePassword(@PathVariable String email, @PathVariable String password){
		LoanForm form=service.searchByEmail(email);
		if(form!=null) {
			form.setPassword(password);
			service.save(form);
			return new Response<LoanForm>(false, "Password Changed", form);
		}else {
			return new Response<LoanForm>(false, "Password not changed", null);
		}
	}
